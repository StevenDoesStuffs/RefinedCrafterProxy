...
class Config(builder: ForgeConfigSpec.Builder) {
    companion object {
        ...
        private const val MAXIMUM_SUCCESSFUL_CRAFTING_UPDATES: String = "maximumSuccessfulCraftingUpdates"

        @JvmStatic
        val CONFIG: dev.stevendoesstuffs.refinedcrafterproxy.Config

        @JvmStatic
        val CONFIG_SPEC: ForgeConfigSpec

        init {
            val (config, configSpec) = ForgeConfigSpec.Builder()
    ...
    init {
        ...
        tiers = builder.define(
            TIERS, {
                ...
                defaultTier.add(MAXIMUM_SUCCESSFUL_CRAFTING_UPDATES, listOf(1, 2, 3, 4, 5))
            ...
            spec.defineInRange(UPGRADES_ENERGY_MULTIPLIER, 1.0, 0.0, Int.MAX_VALUE.toDouble())
            ...
            for (entry in it.valueMap()) {
                ...
                if (config.get<List<Int>>(UPDATE_INTERVALS).size != 5 ||
                    config.get<List<Int>>(MAXIMUM_SUCCESSFUL_CRAFTING_UPDATES).size != 5
                ) return@define false
            }
            true
        }
    ...
    fun getDisplayName(tier: String?): IFormattableTextComponent? {
        if (tier == null || tier == DEFAULT_TIER) return null
        return tiers.get().get<Config?>(tier)?.let { StringTextComponent(it.get(DISPLAY_NAME)) }
            ?: StringTextComponent("ERROR").withStyle(TextFormatting.RED)
    }
    ...
    fun getMaximumSuccessfulCraftingUpdates(tier: String?, upgrades: Int): Int {
        return getTier(tier).get<List<Int>>(MAXIMUM_SUCCESSFUL_CRAFTING_UPDATES)[upgrades.coerceIn(0, 4)]
    }
}
